...
export default function VastuClient({
    ...
    const product = {
        ...
        subtitle: cms?.heroTitle || 'Align your home with ancient wisdom',
                    ...
                    <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
                        ...
                        >
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 mb-10">
                                <Link
                                    href={getPath(cms?.heroCtaLink || '/register?type=vastu')}
                ...
                <div className="container-custom px-6 relative z-10">
                    ...
                    >
                        <div className="group relative cursor-pointer">
                            <Link href={getPath(cms?.uploadButtonLink || '/register?type=vastu')} className="absolute inset-0 z-20" />
                            ...
                            <div className="relative bg-white rounded-[2rem] border-2 border-dashed border-slate-300 group-hover:border-orange-500 transition-all duration-300 p-10 md:p-16 flex flex-col items-center text-center overflow-hidden">
                                ...
                                <button className="px-8 py-4 bg-orange-600 text-white rounded-xl font-bold text-lg shadow-lg shadow-orange-500/20 group-hover:scale-105 transition-transform flex items-center gap-2">
                                    <ImageIcon className="w-5 h-5" /> {cms?.uploadButtonLabel || 'Select Floor Plan to Analyze'}
                                </button>
                            </div>
                        </div>
                    </motion.div>
                    ...
                    {resolvedSteps.map((step, i) => (
                        <div key={i} className="grid lg:grid-cols-2 gap-12 lg:gap-24 items-center group">
                            <div className={`${i % 2 === 1 ? 'lg:order-2' : ''}`}>
                                <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-orange-100 text-orange-600 font-black text-xl mb-6 ring-4 ring-white shadow-lg">{i + 1}</div>
                            ...
                            <div className={`${i % 2 === 1 ? 'lg:order-1' : ''} relative`}>
                                ...
                                <img src={urlFor(step.image).url() || step.image} alt={step.title} className="relative z-10 w-full rounded-[2rem] shadow-xl" />
                            </div>
                        </div>
                    ))}
                </div>
                ...
                {isVideoOpen && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsVideoOpen(false)} className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} onClick={(e) => e.stopPropagation()} className="relative w-full max-w-5xl bg-black rounded-3xl overflow-hidden shadow-2xl aspect-video">
                            ...
                            <iframe className="w-full h-full" src={cms?.heroVideoLink || "https://www.youtube.com/embed/ttZcXOgmrNY?autoplay=1"} title="Vastu Demo Video" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen></iframe>
                        </motion.div>
                    </motion.div>
                )}
            </AnimatePresence>
