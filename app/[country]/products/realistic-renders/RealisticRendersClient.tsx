...
export default function RealisticRendersClient({ cms, resolvedFaqs }: RealisticRendersClientProps) {
    ...
    const lightingStates = {
        ...
        night: {
            ...
            img: cms?.nightImage || NightImg
        }
    };

    const photoFeatures = cms?.photoFeatures || [
    ...
    const qualityTiers = cms?.qualityTiers || [
        { title: 'Preview Draft', resolution: '1K Resolution', time: '< 10 Seconds', useCase: 'Quick layout checks', iconName: 'Monitor' },
        { title: 'Standard HD', resolution: '4K Resolution', time: '~ 2 Minutes', useCase: 'Client presentations', iconName: 'ImageIcon' },
        { title: 'Ultra Photoreal', resolution: '8K Resolution', time: '~ 5 Minutes', useCase: 'Marketing & Print', iconName: 'Sliders' }
    ];

    const audienceRoles = cms?.audienceRoles || [
            ...
            <main>
                    ...
                    <div className="container-custom px-4 relative z-10 w-full">
                        <div className="max-w-4xl mx-auto text-center py-8 lg:py-12 px-6 lg:px-8 rounded-[40px] bg-white/5 backdrop-blur-[4px] border border-white/10 shadow-2xl shadow-black/5">
                            ...
                            >
                                ...
                                <span className="text-xs font-black uppercase tracking-widest text-zlendo-grey-dark">{cms?.heroBadge || 'Next-Gen Rendering Engine'}</span>
                            </motion.div>
                            ...
                            >
                                {cms?.heroTitle || 'See Your Home'}<br />
                                <span className={`text-transparent bg-clip-text bg-gradient-to-r ${timeOfDay === 'night' ? 'from-purple-400 to-blue-400' : 'from-zlendo-teal to-blue-600'}`}>
                                    {cms?.heroTitleHighlight || 'The Way It Will Really Look.'}
                                </span>
                            </motion.h1>
                            ...
                            >
                                {cms?.heroDesc || 'Experience 8K photorealism with intelligent light simulation. No guesswork—just crystal clear, emotional visuals.'}
                            </motion.p>
                        ...
                        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
                            ...
                            <div className="space-y-8">
                                <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-lg font-bold text-xs uppercase ${timeOfDay === 'night' ? 'bg-blue-900/30 text-blue-400' : 'bg-blue-50 text-blue-600'}`}>
                                    ...
                                    <span>{cms?.photoBadge || 'Core Differentiator'}</span>
                                </div>
                                <h2 className="text-4xl md:text-5xl lg:text-6xl font-black leading-tight">
                                    {cms?.photoTitle || 'Images that look like'} <span className="italic text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-purple-500">{cms?.photoTitleHighlight || 'Photographs.'}</span>
                                </h2>
                                <p className={`text-xl font-medium leading-relaxed ${timeOfDay === 'night' ? 'text-slate-400' : 'text-zlendo-grey-medium'}`}>
                                    {cms?.photoDesc || 'Forget cartoonish 3D models. Our advanced ray-tracing engine simulates how light interacts with real-world materials—capturing the softness of fabric, the sheen of hardwood, and the transparency of glass.'}
                                </p>

                                <ul className="space-y-6">
                                    {photoFeatures.map((item: any, i: number) => (
                                        <motion.li
                ...
                <section className={`py-12 lg:py-20 ${timeOfDay === 'night' ? 'bg-slate-950/50' : 'bg-slate-50'}`}>
                    <div className="container-custom px-4 text-center max-w-4xl mx-auto mb-10 lg:mb-16">
                        ...
                        <h2 className="text-3xl md:text-5xl font-black mb-6">
                            {cms?.cameraTitle || 'Good visualization is about'} <span className="text-purple-500">{cms?.cameraTitleHighlight || 'Perspective.'}</span>
                        </h2>
                        <p className={`text-xl font-medium ${timeOfDay === 'night' ? 'text-slate-400' : 'text-zlendo-grey-medium opacity-80'}`}>
                            {cms?.cameraDesc || 'Frame your rooms like a professional photographer. Control height, angle, and focal length to tell the right story.'}
                        </p>
                    </div>

                    <div className="container-custom px-4 grid md:grid-cols-3 gap-6">
                        {cameraCards.map((cam: any, i: number) => (
                            <motion.div
                                key={i}
                                whileHover={{ y: -10 }}
                        ...
                        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
                            <div className="order-2 lg:order-1">
                                <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-lg font-bold text-xs uppercase mb-6 bg-white/50 backdrop-blur-sm border border-black/5 ${lightingStates[timeOfDay].accent}`}>
                                    ...
                                    <span>{cms?.lightingBadge || 'Time-of-day Simulation'}</span>
                                </div>
                                <h2 className={`text-4xl md:text-6xl font-black mb-6 leading-tight ${timeOfDay === 'night' ? 'text-white' : 'text-zlendo-grey-dark'}`}>
                                    {cms?.lightingTitle || 'Light Changes'} <br />
                                    <span className={lightingStates[timeOfDay].accent}>{cms?.lightingTitleHighlight || 'Everything.'}</span>
                                </h2>
                                <p className={`text-xl font-medium mb-10 ${timeOfDay === 'night' ? 'text-slate-300' : 'text-zlendo-grey-medium'}`}>
                                    {cms?.lightingDesc || "Don't build in the dark. Simulate exactly how sunlight moves across your floor plan throughout the day."}
                                </p>

                                {/* Time Toggles */}
                                <div className="space-y-4">
                                    <div className="text-sm font-black uppercase tracking-widest opacity-50 mb-4 text-center lg:text-left">Choose Your Moment</div>
                                    ...
                                    <p className={`mt-6 p-6 rounded-2xl border font-medium text-lg italic transition-all ${timeOfDay === 'night' ? 'bg-white/10 border-white/10 text-slate-200 text-center' : 'bg-white border-white/50 text-slate-600 text-center'}`}>
                ...
                <section className={`py-12 lg:py-20 ${timeOfDay === 'night' ? 'bg-slate-900 border-t border-white/5' : 'bg-white border-t border-slate-100'}`}>
                    ...
                    <div className="container-custom px-4 grid md:grid-cols-3 gap-8">
                        {qualityTiers.map((tier: any, i: number) => {
                            const Icon = IconMapper[tier.iconName] || Monitor;
                            return (
                                <div
                                    key={i}
                                ...
                                >
                                    ...
                                    <div className="text-sm font-bold opacity-60 uppercase tracking-widest mb-6">{tier.resolution}</div>
                                    ...
                                    <ul className="space-y-3">
                                        ...
                                        <li className="flex items-center gap-3 text-sm font-bold opacity-80">
                                            <CheckCircle2 className="w-4 h-4" /> {tier.useCase}
                                        </li>
                                    </ul>
                                </div>
                            );
                        ...
                        <div className="grid md:grid-cols-2 gap-10 lg:gap-12 items-center">
                            <div className="space-y-8">
                                ...
                                <div className="space-y-6">
                                    {audienceRoles.map((user: any, i: number) => (
                    ...
                    <div className="container-custom px-4 text-center relative z-10">
                        <div className="max-w-4xl mx-auto text-center">
                            <h2 className="text-4xl md:text-6xl font-black mb-8 leading-tight">
                                {cms?.ctaTitle || 'Ready to see your future home?'}
                            </h2>
                            <p className="text-xl md:text-2xl font-medium mb-12 opacity-80 max-w-2xl mx-auto">
                                {cms?.ctaDesc || 'Start capturing photorealistic memories of your design today.'}
                            </p>
                            <div className="flex flex-col sm:flex-row justify-center gap-6">
                                ...
                                >
                                    {cms?.ctaLabel || 'Start Rendering Now'}
                                </a>
                            </div>
                        </div>
                    </div>
