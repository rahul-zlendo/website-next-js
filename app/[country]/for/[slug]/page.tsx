...
function getSegmentDefaults(slug: string, country: string): SegmentDefaults {
  ...
  const content = map[slug] ?? map['interior-designers'];

  if (country === 'global') {
    const localizeText = (text: string) => {
      if (!text) return text;
      return text
        .replace(/Built for India's Top/g, 'Built for Top')
        .replace(/Built for Indian/g, 'Built for Professional')
        .replace(/an Indian /gi, 'a ')
        .replace(/Indian market rates/gi, 'local market rates')
        .replace(/Indian market/gi, 'Global market')
        .replace(/Indian /gi, '')
        .replace(/in India/gi, 'worldwide')
        .replace(/₹999/g, '$15')
        .replace(/₹5,000–₹50,000/g, '$50–$500')
        .replace(/₹5,000–₹15,000/g, '$50–$150')
        .replace(/INR pricing starting/gi, 'Pricing starting')
        .replace(/INR pricing/gi, 'Affordable pricing')
        .replace(/INR-denominated/g, 'Affordable')
        .replace(/— not the global market\./g, '.')
        .replace(/No USD conversion, no surprise charges\. /g, '')
        .replace(/12,000\+ Indian/g, '12,000+')
        .replace(/INR/g, 'USD');
    };

    const localizeObject = (obj: any): any => {
      if (typeof obj === 'string') return localizeText(obj);
      if (Array.isArray(obj)) return obj.map(localizeObject);
      if (typeof obj === 'object' && obj !== null) {
        const newObj: any = {};
        for (const key in obj) {
          newObj[key] = localizeObject(obj[key]);
        }
        return newObj;
      }
      return obj;
